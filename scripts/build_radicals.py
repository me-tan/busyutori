...
RADICALS = {
  85:  {"氵": ("さんずい",   "水にかかわる"), "水": ("みず", "水の形が入っている")},
  9:   {"亻": ("にんべん",   "人にかかわる"), "人": ("ひと", "人の形が入っている")},
  64:  {"扌": ("てへん",     "手の動作にかかわる"), "手": ("て", "手の形が入っている")},
  75:  "木",
  149: "言",
  30:  "口",
  32:  "土",
  ...
  140: "艹",
  40:  "宀",
  61:  {"忄": ("りっしんべん","心にかかわる"), "心": ("こころ", "心や気もちにかかわる")},
  154: "貝",
  38:  "女",
  167: "金",
  115: "禾",
  60:  "彳",
  18:  ("刀/刂", "りっとう", "刀で きることにかかわる"),
  118: "竹",
  53:  "广",
  102: "田",
  181: "頁",
  ...
  166: "里",
  104: "疒",
  145: {"衤": ("ころもへん", "衣服にかかわる"), "衣": ("ころも", "衣の形が入っている")},
  113: {"礻": ("しめすへん", "かみさまや いのりにかかわる"), "示": ("しめす", "示の形が入っている")},
  (170, 163): "阝",  # こざとへん(170)・おおざと(163) は形が同じなので出題上は統合
  96:  ("玉/王", "おう", "王・玉の形が入っている"),
  94:  {"犭": ("けものへん", "動物にかかわる"), "犬": ("いぬ", "犬の形が入っている")},
  93:  "牛",
  128: "耳",
  108: "皿",
  142: "虫",
...
NAMES = {
  "木": ("き", "木の形が入っている"), "言": ("ごんべん", "言葉にかかわる"),
  "口": ("くち", "口の形が入っている"), "土": ("つち", "土の形が入っている"),
  "日": ("にち", "日の形が入っている"),
  "月": ("つき", "月の形が入っている"), "辶": ("しんにょう", "みちや すすむことにかかわる"),
  "艹": ("くさかんむり", "くさや植物にかかわる"), "宀": ("うかんむり", "いえ・たてものにかかわる"),
  "貝": ("かい", "お金や たからにかかわる"), "女": ("おんな", "女性にかかわる"),
  "金": ("かね", "金属にかかわる"), "禾": ("のぎへん", "米や麦などの作物にかかわる"),
  "彳": ("ぎょうにんべん", "行くことにかかわる"),
  "竹": ("たけ", "竹にかかわる"), "广": ("まだれ", "建物にかかわる"),
  "田": ("た", "田の形が入っている"), "頁": ("おおがい", "あたまや かおにかかわる"),
  "石": ("いし", "石の形が入っている"), "目": ("め", "目の形が入っている"),
  "車": ("くるま", "車や乗り物にかかわる"), "立": ("たつ", "立つことにかかわる"),
  "米": ("こめ", "米や穀物にかかわる"), "雨": ("あめかんむり", "天気にかかわる"),
  "食": ("しょく", "食べることにかかわる"), "馬": ("うま", "馬にかかわる"),
  "魚": ("うお", "魚にかかわる"), "門": ("もんがまえ", "門や出入口にかかわる"),
  "冫": ("にすい", "氷や つめたさにかかわる"), "穴": ("あなかんむり", "穴にかかわる"),
  "舟": ("ふね", "ふねにかかわる"), "力": ("ちから", "力にかかわる"),
  "巾": ("はば", "ぬのにかかわる"), "攵": ("のぶん", "動作にかかわる"),
  "隹": ("ふるとり", "鳥にかかわる"), "足": ("あし", "足にかかわる"),
  "里": ("さと", "里の形が入っている"), "疒": ("やまいだれ", "やまいや けがにかかわる"),
  "阝": ("こざと・おおざと", "丘や場所にかかわる"),
  "牛": ("うし", "牛にかかわる"), "耳": ("みみ", "耳や きくことにかかわる"),
  "皿": ("さら", "器にかかわる"), "虫": ("むし", "虫にかかわる"),
  "酉": ("ひよみのとり", "さけや はっこうにかかわる"),
}
...
ORIGIN = {
  ...
  "刀/刂": "「刀」が漢字の右側に来るときに形が変わったものが「刂」。",
  "衤": "「衣」が漢字の左側に来るときに形が変わったもの。",
  "玉/王": "「王」は天・地・人をつなぐ大きなまさかり（斧）を持つ者の形からできた字。「玉」は玉を3つひもで貫いた形で、へんに使われるときは点が取れて「王」と同じ形になる。",
...
def bucket_for(radical_number, kanji):
    ...
    if isinstance(spec, tuple):
        return spec[0]  # 名前は1つだが字形が2通り。判定せず両方まとめて1つの部首にする
    if isinstance(spec, str):
        return spec
    # dict: 名前が別々の2つの表示形のうち、実際に含む字形の方を選ぶ
...
def name_meaning(rad):
    for spec in RADICALS.values():
        if isinstance(spec, dict) and rad in spec:
            return spec[rad]
        if isinstance(spec, tuple) and spec[0] == rad:
...
for k, v in joyo.items():
    ...
    if bucket is None or k in bucket:
        continue  # 部首そのものは除外（"刀/刂"の刀・刂自身など）
...
# ---- 出題中に答えが見えていないかの検査 ----
# 部首の説明文（meaning）は対局中ずっと画面に出ている。そこにその部首の
# 出題漢字が入っていると、答えを教えてしまう（「くさかんむり＝草に関係する」など）。
# 「関係する」という言い回し自体も、「関」が門の、「係」がにんべんの答えなので使えない。
# 呼び名（name）も同じ画面に出るので一緒に見る。
leaks = []
for r, d in radicals_out.items():
    pool = {k for ks in d['kanji'].values() for k in ks}
    for field in ('name', 'meaning'):
        for ch in d[field]:
            if ch in pool:
                leaks.append(f"{r}（{d['name']}）「{d[field]}」に答えの「{ch}」が入っている")
if leaks:
    raise SystemExit('出題中に答えが見えてしまう説明文があります:\n  ' + '\n  '.join(leaks))

out = {
    "meta": {
        "description": "ぶしゅとり 部首マスタ",
